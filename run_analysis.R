...
friendlyFeature <- gsub("[\\(\\)]","",features$feature)
friendlyFeature<- gsub("[,-]","_",friendlyFeature)

#Create data tavle
dt <- data.table(mergedData)
#data set with the average of each variable for each activity and each subject. 
tidyData <- dt[, list(V1=mean(V1),V2=mean(V2), V3=mean(V3), V41=mean(V41),
...
#Friendly names
tNames <- names(tidyData)[3:length(names(tidyData))]
tNamesIndex <-  as.numeric(gsub("V","",tNames))
setnames(tidyData, c("activity", "subject", friendlyFeature[tNamesIndex]));
